def viterbi(transition_matrix, emission_matrix, initial_matrix, observation, max_iter=100):
    ...
    # print(num_states)
    delta = [[0 for x in range(num_states)] for y in range(num_emiss)]
    index = [[0 for x in range(num_states)] for y in range(num_emiss-1)]
    ...
    #     viterbi[i][0] = initial_matrix[i] * emission_matrix[i][new_obs[i]]
    
    for i in range(num_states):
            delta[0][i] = initial_matrix[0][i]*emission_matrix[i][new_obs[0]]
    ...
    for t in range(1, num_emiss):
        for i in range(num_states):
            ...
            for j in range(num_states):
                temp.append(delta[t-1][j]*transition_matrix[j][i]*emission_matrix[i][new_obs[t]])
    ...
    #         viterbi[s][t] = trans_prob[state] * emission_matrix[s][new_obs[t]]
    #         path[s][t] = state
    #         iterations += 1
    ...
    result = []
    back_track = delta[num_emiss-1].index(max(delta[num_emiss-1]))
